...
public class MessageUtil implements Serializable {
    ...
    private static final String PACKAGE_PART_SERVICE = ".service.";
    private static final String MODULE_PART_CORE = ".core.";
    private static final Pattern patternPackageAsCoreService = Pattern.compile("^.+\\.service\\.\\w+Service$");
    ...
    public MessageUtil(Class clazz, Locale locale) {
        String nomeBase = clazz.getPackage().getName().concat(I18N_FILENAME);
        if (patternPackageAsCoreService.matcher(clazz.getName()).matches()) {
            nomeBase = nomeBase.replace(PACKAGE_PART_SERVICE, MODULE_PART_CORE);
        }
